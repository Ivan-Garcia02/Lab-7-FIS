...
void Register(System& system) {
  ...
  std::string donation_file{"../BaseData/Donation.txt"};
  System reload{user_file, petitions_file, donation_file};
  system = reload;
...
void show_menu(System& system, int pos) {
  ...
  if (option == 0) {
    ...
    std::string petitions_file{"../BaseData/PETITIONS/petition_general.txt"};
    std::string donation_file{"../BaseData/Donation.txt"};
    System update{user_file, petitions_file, donation_file};
    system = update;
  } else if (option == 3) {
...
void ActualizarBaseDatos(System& system) {
  ...
  for (int i {0}; i < system.GetPetitions().size(); i++) {
    ...
    std::cout << std::to_string(petition_aux.get_donation()) << std::endl;
