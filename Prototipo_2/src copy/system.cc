...
#include "graphics.h"
#include "functions.h"
System::System(const std::string& users, const std::string& petitions, const std::string& donacion) {
  std::ifstream user_file{users};
  std::ifstream petitions_file{petitions};
  std::ifstream donation_file{donacion};
  std::string line{""};
  ...
  while (getline(petitions_file, petition)) {
    ...
    while (getline(petition_name, line)) { 
      int i{0};
      std::string titulo{""}, descripcion{""}, dueno{""}, firmas{""}, donation{""};
      ...
      while (++i < line.size()) {
        donation.push_back(line.at(i));
      }
      int user{std::stoi(dueno)};
      int dinero{std::stoi(donation)};
      
      Petition peticion(titulo, descripcion, std::stoi(firmas), users_.at(user).GetName(), dinero);
  ...
  if (users_[pos].GetPetitionsCreadas().empty()) {
    ...
    while (number_peticion != 0) {
      ...
      if (number_peticion != 0 && number_peticion <= petitions_.size()) {
        ...
        DonatePetition(number_peticion);
        SharePetition(number_peticion);
      }
      std::cout << "Pulsa una tecla para continuar: ";
      std::string esperar;
      std::getline(std::cin, esperar);
...
void System::DonatePlataforma() {
  ...
  cantidad_donada_ += money;
  std::cout << "Muchas gracias por la donacion. \nAcabas de donar: " << money << "€ \nDorf ha recaudado: " << cantidad_donada_ << "€" << std::endl;
}
